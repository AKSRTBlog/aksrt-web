...
import { k as buildArchiveGroups, l as formatLongDate, j as fetchAllPublicArticles } from "./api-9xEAR-2s.js";
          ...
          ssrRenderList(monthGroup.items, (article) => {
            _push(ssrRenderComponent(_component_NuxtLink, {
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  _push2(`<div${_scopeId}><p class="text-sm font-semibold text-[var(--blog-ink)]"${_scopeId}>${ssrInterpolate(article.title)}</p><p class="mt-1 text-xs text-[var(--blog-subtle)]"${_scopeId}>${ssrInterpolate(article.categories?.[0]?.name || "Uncategorized")}</p></div><span class="text-xs text-[var(--blog-subtle)]"${_scopeId}>${ssrInterpolate(unref(formatLongDate)(article.publishedAt))}</span>`);
                } else {
                  return [
                    createVNode("div", null, [
                      createVNode("p", { class: "text-sm font-semibold text-[var(--blog-ink)]" }, toDisplayString(article.title), 1),
                      createVNode("p", { class: "mt-1 text-xs text-[var(--blog-subtle)]" }, toDisplayString(article.categories?.[0]?.name || "Uncategorized"), 1)
                    ]),
...
//# sourceMappingURL=archive-C9Igc78R.js.map
