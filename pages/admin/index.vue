  ...
  return [
    {
      marker: '文',
      label: '文章总数',
      value: formatAdminNumber(dashboardState.articles.length),
      change: `${publishedArticles} 已发布`,
    },
    {
      marker: '发',
      label: '已发布文章',
      value: formatAdminNumber(publishedArticles),
      change: `${draftArticles} 草稿中`,
    },
    {
      marker: '评',
      label: '待审核评论',
      value: formatAdminNumber(pendingComments),
      change: '需要处理',
    },
    {
      marker: '媒',
      label: '媒体资源',
      value: formatAdminNumber(activeMedia),
      change: `${dashboardState.media.length} 总计`,
    },
  ]
})
...
<template>
  <div class="w-full max-w-full space-y-4 overflow-x-hidden sm:space-y-6">
    <AdminPageHeader title="后台仪表盘" description="查看文章、评论和媒体资源的最新状态。">
      <template #actions>
        <NuxtLink class="admin-button-primary w-full justify-center sm:w-auto" :to="adminPaths.articleCreate">
          新建文章
        </NuxtLink>
        <NuxtLink class="admin-button-secondary w-full justify-center sm:w-auto" :to="adminPaths.media">
          上传媒体
        </NuxtLink>
      </template>
    </AdminPageHeader>
    ...
    <template v-else>
      ...
      <section class="grid gap-4 sm:gap-6 xl:grid-cols-2">
        <div class="space-y-4 sm:space-y-6">
          <div class="admin-card overflow-hidden p-4 sm:p-6">
            <div class="mb-4 flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
              ...
              <span class="hidden shrink-0 text-xs font-medium uppercase tracking-[0.18em] text-slate-400 sm:inline">
                Shortcuts
              </span>
            </div>
            ...
            <div class="grid gap-3">
              <NuxtLink
                class="flex w-full min-w-0 items-center justify-between gap-3 rounded-[4px] border border-[var(--admin-border)] px-3 py-3 transition hover:border-blue-200 hover:bg-blue-50/50 sm:px-4 sm:py-4"
                :to="adminPaths.comments"
              >
                <div class="min-w-0 flex-1">
                  <p class="text-sm font-semibold text-slate-900">评论管理</p>
                  <p class="mt-0.5 break-words text-xs text-slate-500">审核、通过或处理最新评论。</p>
                </div>
              ...
              >
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-0.5 break-words text-xs text-slate-500">维护首页与侧边栏的展示内容。</p>
                </div>
                <svg class="h-4 w-4 shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
              ...
              >
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-0.5 break-words text-xs text-slate-500">修改 SEO、SMTP 与存储配置。</p>
                </div>
                <svg class="h-4 w-4 shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                </svg>
          ...
          <div class="admin-card overflow-hidden p-4 sm:p-6">
            <div class="mb-4 flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
              ...
              <span class="max-w-full break-all text-xs text-slate-500 sm:max-w-[60%] sm:text-sm">
          ...
          <div class="admin-card overflow-hidden p-4 sm:p-6">
            ...
            <div v-if="recentArticles.length" class="space-y-3">
              <div
                ...
                :key="article.id"
                class="flex flex-col items-start gap-2 rounded-[4px] border border-[var(--admin-border)] px-3 py-2.5 sm:flex-row sm:items-center sm:gap-4 sm:px-4 sm:py-3"
              >
                <div class="w-full min-w-0 flex-1">
                  ...
                  <p class="mt-0.5 break-words text-xs text-slate-500 sm:truncate">
                    {{ article.categories?.[0]?.name || adminText.uncategorized }} / {{ formatAdminDate(article.createdAt) }}
                  </p>
                </div>
          ...
          <div class="admin-card overflow-hidden p-4 sm:p-6">
            ...
            <div v-if="recentComments.length" class="space-y-3">
              <div
                ...
                :key="comment.id"
                class="w-full min-w-0 rounded-[4px] border border-[var(--admin-border)] px-3 py-3 sm:px-4 sm:py-4"
              >
                ...
                <p class="mt-1 text-xs text-slate-400">{{ comment.article.title }}</p>
                <p class="mt-2 line-clamp-3 break-words text-sm text-slate-500">{{ comment.content }}</p>
